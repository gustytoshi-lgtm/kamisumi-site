...
| I-022 | High | `.git` ACL により stage/commit/push ができない | `git add ...` | `fatal: Unable to create 'C:/dev/sites/kamisumi-site/.git/index.lock': Permission denied` でドキュメント更新や実装変更をコミットできない | 作業内容は作業ツリーに保持。Git操作前に状態を再確認 | `.git` の明示 DENY ACL を復旧し、`git status` / `git add` / `git commit` を再実行 | 運用 | Open |
